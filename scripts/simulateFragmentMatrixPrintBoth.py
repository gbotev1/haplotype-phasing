	...
	with open(args.matrixout,"w") as outLong:
		...
		if fragmentlength == None or int(fragmentlength) == width: #No gaps at the ends of fragments
			...
			if len(startpoints) < remaining: 
				for k in xrange(remaining-len(startpoints)):
					startpoints.append(k) 
			H0 = startpoints[0::2]
			H1 = startpoints[1::2]
...
if errorrate < 0.0 or errorrate > 1.0 or gaprate < 0.0 or gaprate > 1.0:
	...
	sys.exit()
if args.shortfmt == False:
	outfile = None
	outfile1 = args.matrixout
else:
	outfile = args.matrixout + "-short"
	outfile1 = args.matrixout
if k == 2:
	H = diploid(args.fragmentlength,height, width, gaprate, errorrate,k,fractionh0,outfile,outfile1)
else:

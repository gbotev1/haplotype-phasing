# Written by Charlotte Darby cdarby@jhu.edu
# Edited by Georgie Botev gbotev1@jhu.edu
# updated 9-30-17
...
def kploid(fragmentlength, height, width, gaprate, errorrate, k, out):
    ...
    for _ in xrange(k):
        haplotypes.append([])
    for _ in xrange(
            width):  # for each site, there is at least one 0 and at least one 1, but others are randomly selected
    ...
    with open(out, "w") as F:
        ...
        # Write down the number of rows and columns for post-processing
        F.write("%d\n" % height)
        F.write("%d\n" % width)
        ...
        if fragmentlength == None or fragmentlength == width:  # No gaps at the ends of fragments
            for h in haplotypes:
                for _ in xrange(int(1.0 * height / k)):
                    fragment = ["-" if random.random() < gaprate else str(h[i]) if random.random() > errorrate else str(
                        abs(1 - h[i])) for i in xrange(width)]
                    F.write("".join(fragment))
                    F.write("\n")

            for h in xrange(height - k * int(
                                    1.0 * height / k)):  # Number of fragments may not be divisible by k, these are the "leftovers"
                fragment = [
                    "-" if random.random() < gaprate else str(haplotypes[h][i]) if random.random() > errorrate else str(
                        abs(1 - haplotypes[h][i])) for i in xrange(width)]
                F.write("".join(fragment))
                F.write("\n")

        else:
            fragmentlength = width if int(fragmentlength) > width else int(
                fragmentlength)  # There are gaps at the ends of fragments
            ...
            for h in haplotypes:
                ...
                if persite > 0:
                    for i in xrange(width - fragmentlength + 1):  # number of gaps to put in front
                        for p in xrange(persite):
                            fragment = ["-" for _ in xrange(i)] + ["-" if random.random() < gaprate else str(
                                h[ch]) if random.random() > errorrate else str(abs(1 - h[ch])) for ch in
                                                                   xrange(i, i + fragmentlength)] + ["-" for _ in
                                                                                                     xrange(
                                                                                                         i + fragmentlength,
                                                                                                         width)]
                            F.write("".join(fragment))
                ...
                for i in random.sample(range(width - fragmentlength + 1), remaining):  # number of gaps to put in front
                    fragment = ["-" for _ in xrange(i)] + [
                        "-" if random.random() < gaprate else str(h[ch]) if random.random() > errorrate else str(
                            abs(1 - h[ch])) for ch in xrange(i, i + fragmentlength)] + ["-" for _ in
                                                                                        xrange(i + fragmentlength,
                                                                                               width)]
                    F.write("".join(fragment))
                    F.write("\n")

            remaining = height - k * int(1.0 * height / k)
            for (i, h) in zip(random.sample(range(width - fragmentlength + 1), remaining),
                              haplotypes):  # number of gaps to put in front
                fragment = ["-" for _ in xrange(i)] + [
                    "-" if random.random() < gaprate else str(h[ch]) if random.random() < errorrate else str(
                        abs(1 - h[ch])) for ch in xrange(i, i + fragmentlength)] + ["-" for _ in
                                                                                    xrange(i + fragmentlength, width)]
                F.write("".join(fragment))
                F.write("\n")
    return haplotypes
    ...
    with open(out, "w") as F:
        ...
        if fragmentlength == None or fragmentlength == width:  # No gaps at the ends of fragments
            for _ in xrange(int(height * fractionh0)):  # h0
                fragment = [
                    "-" if random.random() < gaprate else str(haplotype[i]) if random.random() > errorrate else str(
                        abs(1 - haplotype[i])) for i in xrange(width)]
                F.write("".join(fragment))
                F.write("\n")
            ...
            if persite > 0:
                for i in xrange(width - fragmentlength + 1):  # number of gaps to put in front
                    for p in xrange(persite):
                        fragment = ["-" for _ in xrange(i)] + ["-" if random.random() < gaprate else str(
                            haplotype[ch]) if random.random() > errorrate else str(abs(1 - haplotype[ch])) for ch in
                                                               xrange(i, i + fragmentlength)] + ["-" for _ in xrange(
                            i + fragmentlength, width)]
                        F.write("".join(fragment))
                        F.write("\n")
            for i in random.sample(range(width - fragmentlength + 1), remaining):  # number of gaps to put in front
                fragment = ["-" for _ in xrange(i)] + [
                    "-" if random.random() < gaprate else str(haplotype[ch]) if random.random() > errorrate else str(
                        abs(1 - haplotype[ch])) for ch in xrange(i, i + fragmentlength)] + ["-" for _ in
                                                                                            xrange(i + fragmentlength,
                                                                                                   width)]
                F.write("".join(fragment))
                F.write("\n")

            persite = int(1.0 * (height - int(height * fractionh0)) / (
                width - fragmentlength + 1))  # h1 = opposite of h0; change to < errorrate
...
parser = argparse.ArgumentParser(
    description='simulateKploidMatrix.py --matrixout --hapout [--height --width --gaprate --errorrate --fractionh0 --fragmentlength --k]')
parser.add_argument('--matrixout', help='output filename for the fragment matrix', required=True)
parser.add_argument('--hapout',
                    help='output filename for the actual haplotype - depending on error rate, may not be the haplotypes corresponding to the MEC score',
                    required=True)
...
parser.add_argument('--gaprate', help="fraction of entries within fragments that are gaps (randomly distributed)",
                    required=False, default=0.0)
parser.add_argument('--errorrate',
                    help="fraction of matrix entries that have errors (randomly distributed) given that there is not a gap at that point",
                    required=False, default=0.0)
parser.add_argument('--fractionh0', help="fraction of rows that come from 'hap 0' (only applies to k=2)",
                    required=False, default=0.5)
# model unevenness of haplotype in k-ploid later?
parser.add_argument('--fragmentlength', help="length of fragments between first non-gap and last non-gap",
